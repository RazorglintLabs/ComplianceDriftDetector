# ComplianceDriftDetector License Tiers

## Monthly Licenses

| Tier | Price | Best For |
|---|---:|---|
| Starter | $49/mo | Solo/internal evaluation |
| Team | $149/mo | Internal compliance/security teams |
| Consultant | $299/mo | vCISOs, SOC 2/ISO/GRC consultants |

---

### Starter — $49/month

For solo practitioners and internal evaluation.

- Self-service local kit
- Updates while subscribed
- Template packs
- Email support / async questions
- 1 assisted interpretation question per month

### Team — $149/month

For SaaS teams, GRC teams, small security teams.

- Everything in Starter
- Internal team use
- Monthly template updates
- 1 assisted report interpretation per month
- Priority support
- Permission to use reports internally for audit prep

### Consultant — $299/month

For GRC consultants, SOC 2 consultants, ISO consultants, vCISOs.

- Use with multiple client engagements
- Reports carry "powered by Razorglint Labs" attribution unless separately licensed
- 2 assisted report interpretations per month
- Reusable client intake templates
- Commercial consulting use allowed

## Boundaries

This software provides deterministic policy-to-behavior drift detection and tamper-evident readiness evidence.

It does not provide compliance certification, legal advice, audit certification, regulatory approval, or guaranteed violation detection.

Commercial use beyond the selected tier requires written permission from Razorglint Labs / TCOG Collective LLC.

## Policies

- [Terms of Service](TERMS_OF_SERVICE.md)
- [Privacy Policy](PRIVACY_POLICY.md)
- [Refund and Cancellation Policy](REFUND_AND_CANCELLATION_POLICY.md)
- [Notice](NOTICE.md)
